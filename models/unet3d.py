...
class TriVol_Encoder(nn.Module):
    def __init__(self, in_channels, out_channels, nf, num_groups):
        super().__init__()
    ...
    def forward(self, voxels):
        ...
        vx = voxels.reshape(B, C, P, G, S, S).permute(0, 1, 3, 2, 4, 5).reshape(B, C*G, P, S, S)  # (B, C, P, S, S)
        vy = voxels.reshape(B, C, S, P, G, S).permute(0, 1, 4, 2, 3, 5).reshape(B, C*G, S, P, S)  # (B, C, S, P, S)
        vz = voxels.reshape(B, C, S, S, P, G).permute(0, 1, 5, 2, 3, 4).reshape(B, C*G, S, S, P)  # (B, C, S, S, P)
